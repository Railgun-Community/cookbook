...
export abstract class UniswapSwapRecipeCore extends Recipe {
  protected quote: Optional<UniswapSwapQuoteData>;
  ...
  getLatestQuote(): Optional<UniswapSwapQuoteData> {
    return this.quote;
  }
  ...
  getBuySellAmountsFromRecipeOutput(
    ...
    try {
      ...
      if (swapStepOutput.name !== 'Uniswap Exchange Swap') {
        throw new Error('Expected step output 3 to be Uniswap Exchange Swap.');
      }
