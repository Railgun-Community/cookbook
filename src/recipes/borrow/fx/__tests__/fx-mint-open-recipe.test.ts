...
describe('validatePoolFlow', () => {
  it('throws for wstETH-Long without swapQuote', () => {
    expect(() => validatePoolFlow('wstETH-Long', undefined)).to.throw(/wstETH-Long/);
  });

  it('throws for WBTC-Long with swapQuote', () => {
    expect(() => validatePoolFlow('WBTC-Long', fakeSwapQuote)).to.throw(/WBTC-Long/);
  });
  ...
  it('trusts custom pool refs (does not throw)', () => {
    const customPool = {
      ...
      collateralToken: '0x000000000000000000000000000000000000cafe' as `0x${string}`,
